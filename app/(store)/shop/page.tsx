...
function ProductGrid({ products }: { products: StorefrontProduct[] }) { return <div className="mt-12 grid grid-cols-2 gap-x-4 gap-y-12 sm:gap-x-6 lg:grid-cols-4 lg:gap-x-8 lg:gap-y-16">{products.map((product) => <Link className="group min-w-0" href={`/products/${product.slug}`} key={product.id}><div className="relative aspect-[4/5] overflow-hidden bg-[#eeece6]">{product.imageUrl ? <img alt={product.imageAlt ?? product.title} className="size-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.02]" src={product.imageUrl} /> : <div aria-label={product.imageAlt ?? product.title} className="size-full bg-[#e6e2d9]" role="img" />}</div><div className="mt-4"><h2 className="text-sm leading-tight tracking-[-.015em]">{product.title}</h2><p className="font-editorial mt-1 text-sm text-black/55">{product.categoryName ?? "Collection"}</p><p className="mt-2 text-sm">{formatCurrency(product.pricePaise)}</p></div></Link>)}</div>; }
