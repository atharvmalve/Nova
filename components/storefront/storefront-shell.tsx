    ...
    <header className="border-b border-border/70 bg-background/90 backdrop-blur">
      <nav aria-label="Main navigation" className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-8">
        <Link className="flex items-center gap-2 text-lg font-bold tracking-tight" href="/">
          <span className="grid size-8 place-items-center rounded-lg bg-primary text-primary-foreground">N</span>
          NOVA
        </Link>
  ...
  return (
    <footer className="border-t bg-muted/30">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-12 sm:px-8 md:grid-cols-[1.5fr_1fr_1fr]">
        <div>
          <Link className="text-lg font-bold tracking-tight" href="/">NOVA</Link>
          <p className="mt-3 max-w-xs text-sm leading-6 text-muted-foreground">Considered pieces for the everyday, chosen to last beyond the season.</p>
        </div>
        <div>
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-2 border-t px-5 py-5 text-xs text-muted-foreground sm:flex-row sm:justify-between sm:px-8">
        <span>© {new Date().getFullYear()} NOVA. All rights reserved.</span><span>Made for everyday rituals.</span>
      </div>
    </footer>
  );
}
